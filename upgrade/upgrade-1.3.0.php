...
{
    $queries[] = 'ALTER TABLE ' . _DB_PREFIX_ . SamedayService::TABLE_NAME . '
            ADD `service_optional_taxes` TEXT';

    $samedayOpenPackageTableName = _DB_PREFIX_ . SamedayOpenPackage::TABLE_NAME;
    if (false === (new SamedayGeneralQueryHelper())->isTableExists($samedayOpenPackageTableName)) {
        $queries[] = 'CREATE TABLE `' . $samedayOpenPackageTableName ."` (
